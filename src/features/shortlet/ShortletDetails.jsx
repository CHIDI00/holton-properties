...
const ShortletDetails = () => {
	// Get the shortletId from the URL parameters
	const { shortletId } = useParams();

	// Find the shortlet that matches the shortletId
	const shortlet = shortletData.find((p) => p.id === parseInt(shortletId));

	// If shortlet not found, display a message
	if (!shortlet) {
		return (
			<div className="container mx-auto px-4 py-20 text-center mt-20 w-full h-screen">
				<h2 className="text-4xl font-bold text-red-600">shortlet Not Found</h2>
		...
		<div className="w-screen bg-slate-50">
			<div className="w-full">
				<div className="w-full bg-blue-950 py-20 md:py-32 flex flex-col justify-center items-center mt-28 text-white">
					<h2 className="text-5xl mb-4 font-semibold">shortlet Details</h2>
					<p className="text-2xl">
						<a href="" className="text-yellow-700 ">
							<NavLink to="/">Home </NavLink>
						</a>
						/ View this shortlet details
					</p>
				</div>
			</div>
				...
				<div className="w-full p-3 flex justify-between items-center pb-20 mb-20 border-b-2 border-gray-300">
					<div className="">
						...
						<p className="text-xl font-semibold text-gray-500 flex justify-start items-center gap-3">
							<FaLocationDot /> {shortlet.location}
						</p>
					</div>
					<div className="text-3xl font-semibold">
						<p>N{shortlet.price}</p>
					</div>
				</div>
				...
				<div className="w-full p-3 flex flex-col md:flex-row justify-between items-start gap-5 md:gap-10">
					<div className="w-full md:w-[65%]">
						<div className="bg-white p-6 rounded-lg md:rounded-xl mb-10">
							<ImageSlider
								imageList={shortletData[shortletId - 1].detailImages}
							/>
						</div>
						<div className="w-full p-7 bg-white rounded-md my-10">
							...
							<div className="w-full flex justify-between items-center">
								<div className="text-2xl flex flex-col justify-center items-center">
									<FaBed size={20} />
									<p className="text-2xl pt-4">{shortlet.beds} Bedroom</p>
								</div>
								<div className="text-2xl flex flex-col justify-center items-center">
									<FaBath size={20} />
									<p className="text-2xl pt-4">{shortlet.baths} Bathroom</p>
								</div>
								<div className="text-2xl flex flex-col justify-center items-center">
									<FaCar size={20} />
								...
								<div className="text-2xl flex flex-col justify-center items-center">
									<FaBuilding size={20} />
									<p className="text-2xl pt-4">{shortlet.area}</p>
								</div>
							</div>
						</div>
						...
						<div className="w-full p-7 bg-white rounded-md my-10">
							...
							<p className="text-2xl text-gray-400">
								{shortletData[shortletId - 1].description}
							</p>
						</div>

						<div className="w-full p-7 bg-white rounded-md my-10">
							<p className="text-4xl font-semibold mb-10">shortlet Address</p>
						...
						<div className="w-full p-7 bg-white rounded-md my-10">
							<p className="text-4xl font-semibold mb-10">shortlet Details</p>
